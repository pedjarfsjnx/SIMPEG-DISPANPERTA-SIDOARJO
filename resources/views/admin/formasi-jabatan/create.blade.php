...
@section('title', 'Tambah Formasi Jabatan - Admin SIMPEG')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200">
        <div>
            <div class="flex items-center space-x-2">
                <span class="px-2.5 py-0.5 bg-emerald-100 text-emerald-800 font-bold rounded-lg text-[10px] uppercase tracking-wider">Struktur Organisasi & Posisi</span>
                <span class="text-slate-400">&bull;</span>
                <span class="text-xs text-slate-500 font-medium">Slot Formasi Instansi</span>
            </div>
            <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight mt-1">Tambah Formasi Jabatan Baru</h2>
            <p class="text-sm text-slate-500 mt-0.5">Daftarkan slot posisi jabatan struktural, fungsional, atau pelaksana.</p>
        </div>
    ...
    <!-- Main Card Form -->
    <form method="POST" action="{{ route('admin.formasi-jabatan.store') }}" class="bg-white p-6 sm:p-10 rounded-2xl border border-slate-200 shadow-md space-y-8">
        @csrf
                ...
                <div>
                    ...
                    <select name="bidang_id" class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/20 py-2.5 px-3.5 bg-white shadow-2xs">
                        ...
                        @foreach($bidangList as $bid)
                            <option value="{{ $bid->id }}" {{ old('bidang_id') == $bid->id ? 'selected' : '' }}>
        ...
        <div class="space-y-5">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       required 
                       placeholder="Contoh: Kepala Bidang Ketahanan Pangan" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" 
                           ...
                           placeholder="Contoh: 11, 9, 7 (angka kelas)" 
                           class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/20 py-2.5 px-3.5 shadow-2xs">
                </div>
                ...
                <div>
                    ...
                    <select name="status_formasi" required class="w-full text-sm rounded-xl border-slate-300 focus:border-emerald-700 focus:ring-2 focus:ring-emerald-700/20 py-2.5 px-3.5 bg-white shadow-2xs">
                        <option value="kosong" {{ old('status_formasi') == 'kosong' ? 'selected' : '' }}>Kosong / Lowong (Siap Diisi)</option>
                        <option value="terisi" {{ old('status_formasi') == 'terisi' ? 'selected' : '' }}>Terisi (Sudah Ada Pejabat)</option>
                    </select>
                </div>
            </div>
        </div>
