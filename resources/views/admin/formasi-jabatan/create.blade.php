...
<div class="max-w-3xl space-y-5 text-xs">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-900">Tambah Formasi Jabatan Baru</h2>
            <p class="text-slate-500">Daftarkan slot posisi jabatan instansi.</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <form method="POST" action="{{ route('admin.formasi-jabatan.store') }}" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_jabatan" value="{{ old('nama_jabatan') }}" required placeholder="Contoh: Kepala Bidang Ketahanan Pangan" class="w-full text-xs rounded-xl border-slate-300 focus:border-emerald-700 py-2.5">
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block font-semibold text-slate-700 mb-1">Kelas Jabatan</label>
                    <input type="text" name="kelas_jabatan" value="{{ old('kelas_jabatan') }}" placeholder="Contoh: 11, 9, 7" class="w-full text-xs rounded-xl border-slate-300 focus:border-emerald-700 py-2.5">
                </div>
                ...
                <div>
                    ...
                    <select name="status_formasi" required class="w-full text-xs rounded-xl border-slate-300 focus:border-emerald-700 py-2.5">
                        <option value="kosong" {{ old('status_formasi') == 'kosong' ? 'selected' : '' }}>Kosong / Lowong</option>
                        <option value="terisi" {{ old('status_formasi') == 'terisi' ? 'selected' : '' }}>Terisi</option>
                    </select>
                </div>
            </div>
