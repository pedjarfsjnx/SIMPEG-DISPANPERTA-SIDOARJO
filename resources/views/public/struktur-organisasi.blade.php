...
@section('title', 'Struktur Organisasi & Eselon - Dinas Pangan dan Pertanian Sidoarjo')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-3 border-b border-slate-200">
        <div>
            <h2 class="text-xl font-bold text-slate-900 tracking-tight">Struktur Organisasi & Hierarki Eselon</h2>
            <p class="text-xs text-slate-500 mt-0.5">Bagan organisasi resmi, rekapitulasi Eselon kepemimpinan, dan alokasi personel Dinas Pangan dan Pertanian Kabupaten Sidoarjo.</p>
        </div>
        <a href="{{ route('public.pegawai.index') }}" class="px-4 py-2 bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-bold rounded-lg shadow-sm whitespace-nowrap">
            Cari Pegawai &rarr;
        </a>
    </div>

    <!-- Section 1: Foto Bagan Struktur Organisasi Resmi -->
    <div class="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
                <h3 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Bagan Visual Struktur Organisasi</h3>
            ...
            @else
                <div class="text-center space-y-2 max-w-md py-6">
                    ...
                    <p class="text-xs text-slate-500 leading-relaxed">
                        Simpan foto bagan Anda dengan nama <code class="bg-slate-200 px-1.5 py-0.5 rounded font-mono text-slate-900 font-bold">struktur-organisasi.png</code> di folder:<br>
                        <span class="font-mono text-emerald-800 font-bold text-[11px]">c:\xampp\htdocs\dispanperta\public\images\</span>
                    </p>
                </div>
            @endif
        </div>
    ...
    <div class="space-y-5">
        <div class="border-b border-slate-200 pb-2">
            <h3 class="text-base font-bold text-slate-900 uppercase tracking-wider">Pejabat Berdasarkan Tingkatan Eselon</h3>
            <p class="text-xs text-slate-500">Nama pejabat, posisi jabatan, dan profil detail per tingkat kepemimpinan.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl border border-emerald-800 p-5 shadow-sm space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div>
                        ...
                        <h4 class="text-sm font-bold text-slate-900 mt-1">Eselon II.b (Kelas 14 - Kadis)</h4>
                    </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="bg-slate-50 border border-slate-200/80 rounded-lg p-3.5 flex items-center justify-between gap-3 text-xs">
                        <div>
            ...
            <div class="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm space-y-4">
                <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                    <div>
                        <span class="text-[10px] font-bold uppercase tracking-wider bg-amber-500 text-slate-950 px-2 py-0.5 rounded">Administrator</span>
                        <h4 class="text-sm font-bold text-slate-900 mt-1">Eselon III.a (Kelas 11 - 13: Sekdin & Kabid)</h4>
                    </div>
                    <span class="text-xs font-bold text-emerald-800 bg-emerald-100 px-2.5 py-1 rounded-md">
                ...
                <div class="space-y-2.5 max-h-[300px] overflow-y-auto pr-1 text-xs">
                    @forelse($eselon3a as $peg)
                    <div class="bg-slate-50 border border-slate-200/80 rounded-lg p-3 flex items-center justify-between gap-3">
                        <div>
        ...
        <div class="bg-white rounded-xl border border-slate-200/80 p-5 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
                ...
                <div class="bg-slate-50 border border-slate-200/80 rounded-lg p-3 flex flex-col justify-between space-y-2">
                    <div>
                        <div class="font-bold text-slate-900">{{ $peg->nama }}</div>
                        <div class="text-emerald-800 font-semibold text-[11px] mt-0.5">{{ $peg->formasiJabatan?->nama_jabatan }}</div>
                ...
                @empty
                <div class="col-span-3 text-xs text-slate-400 italic">Data Eselon III.b belum terdata.</div>
                @endforelse
            </div>
        </div>
    ...
    <div class="space-y-5">
        <div class="border-b border-slate-200 pb-2">
            <h3 class="text-base font-bold text-slate-900 uppercase tracking-wider">Distribusi & Daftar Personel Per Unit Kerja</h3>
            <p class="text-xs text-slate-500">Alokasi pegawai pada Dinas Induk, Bidang Struktural, UPTD RPH, dan UPTD Lab.</p>
        </div>

        <div class="space-y-6">
            @foreach($unitKerjaWithBidang as $unit)
            <div class="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden text-xs">
                <div class="bg-slate-900 text-white p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b-2 border-amber-500">
                    <div>
                        <span class="text-[10px] font-semibold uppercase tracking-wider text-amber-400">[{{ $unit->tipe }}]</span>
                        <h3 class="text-base font-bold">{{ $unit->nama }}</h3>
                    </div>
                    <div class="flex items-center gap-2 text-xs">
                        <a href="{{ route('public.pegawai.index', ['unit_kerja_id' => $unit->id]) }}" class="px-3 py-1 bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold rounded-md shadow-xs text-xs transition">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div class="bg-slate-50 border border-slate-200/80 rounded-xl p-4 space-y-3">
                                <div class="flex items-center justify-between border-b border-slate-200 pb-2">
                                    ...
                                    <span class="text-[11px] text-emerald-800 font-bold bg-emerald-100 px-2 py-0.5 rounded-md whitespace-nowrap">
                                ...
                                <ul class="text-xs text-slate-700 divide-y divide-slate-200/80 max-h-56 overflow-y-auto pr-1">
                                    ...
                                    <li class="py-1.5 flex justify-between items-center">
                                        ...
                                        <span class="text-slate-700 bg-white px-2 py-0.5 rounded font-mono text-[10px] font-semibold border border-slate-200 whitespace-nowrap ml-2">
                                            {{ $peg->kategori?->nama }}
                                        </span>
                                    </li>
                                    @endforeach
                                ...
                                <div class="bg-slate-50 border border-slate-200/80 rounded-lg p-3 flex flex-col justify-between space-y-2">
                                    <div>
                                        <div class="flex items-start justify-between gap-1">
                                            <a href="{{ route('public.pegawai.show', $peg->id) }}" class="font-bold text-slate-900 hover:text-emerald-800 leading-snug">
                                                {{ $peg->nama }}
                                            </a>
                                            <span class="text-[10px] font-bold px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded whitespace-nowrap">
                                                {{ $peg->kategori?->nama }}
                                            </span>
                                        </div>
