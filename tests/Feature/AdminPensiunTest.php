...
class AdminPensiunTest extends TestCase
{
    ...
    #[Test]
    public function filter_rentang_tahun_menyaring_rekap(): void
    {
        $this->pegawaiDenganPensiun('1968-05-20'); // pensiun 2026
        $this->pegawaiDenganPensiun('1969-06-15'); // pensiun 2027
        $this->pegawaiDenganPensiun('1970-02-10'); // pensiun 2028
        $this->pegawaiDenganPensiun('1972-04-12'); // pensiun 2030
        ...
        $tahunHasil = array_map(fn ($item) => $item->tahun, $list->items());
        $this->assertContains(2026, $tahunHasil);
        $this->assertContains(2027, $tahunHasil);
        $this->assertNotContains(2028, $tahunHasil);
        $this->assertNotContains(2030, $tahunHasil);
    }

    #[Test]
    public function cetak_menampilkan_halaman_cetak_dengan_rentang_tahun(): void
    {
        $p1 = $this->pegawaiDenganPensiun('1968-05-20'); // pensiun 2026
        $p2 = $this->pegawaiDenganPensiun('1969-06-15'); // pensiun 2027
        $p3 = $this->pegawaiDenganPensiun('1972-04-12'); // pensiun 2030

        $response = $this->actingAs($this->user)
            ->get(route('admin.pensiun.cetak', [
                'tahun_mulai' => 2026,
                'tahun_selesai' => 2027,
            ]));

        $response->assertOk()
            ->assertSee('Tahun 2026 s.d. 2027')
            ->assertSee($p1->nama)
            ->assertSee($p2->nama)
            ->assertDontSee($p3->nama);
    }
